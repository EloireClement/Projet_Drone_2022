...
species drone {
	...
	reflex rentrer when: energie > 0 and rentrer_base {
		...
		drone_grid cell_est <- drone_grid({my_cell.grid_x +1 , my_cell.grid_y });
		drone_grid cell_ouest <- drone_grid({my_cell.grid_x - 1  , my_cell.grid_y });
		drone_grid cell_nord <- drone_grid({my_cell.grid_x , my_cell.grid_y -1});
		drone_grid cell_sud <- drone_grid({my_cell.grid_x , my_cell.grid_y +1});
	    ...
	    direction <- int(liste index_of (liste min_of (each)));
	    switch (direction mod 360){
			//Gauche
			match 90 {  
				if (my_cell.grid_x - 1 != -1){
...
species base {
	...
	reflex signal {
		...
		loop energie over: energies_drones {
			...
			if (energie < max_charge * seuil_rappel) {
				ask drone[i] {
					rentrer_base <- true;
				}
			}
	...
	// Define inspectors, browsers and displays here
		display Population_information refresh: every(10#cycles) {
				chart "Species evolution" type: series size: {1,0.5} position: {0, 0} {
					data "number_of_drone" value: drone count (each.energie > 0) color: #blue;
